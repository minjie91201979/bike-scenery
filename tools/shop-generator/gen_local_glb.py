#!/usr/bin/env python3
"""
gen_local_glb.py — 纯本地生成 faceted low-poly 店铺 GLB 模型
不依赖任何外部 API 或 Blender，直接写入 public/assets/models/
用法: python3 gen_local_glb.py [id1,id2,...]  不加参数生成全部
"""
import struct
import json
import os
...
def write_glb(path: str, scene: dict, bin_data: bytes):
    """Write a GLB file from a scene dict and binary payload."""
...
def build_primitive(primitive: dict) -> Tuple[dict, bytes]:
    """Build GLTF primitive and binary data."""
    positions = primitive['positions']
    raw_normals = primitive.get('normals', [])
    indices = primitive.get('indices', [])

    # Flatten normals (list of tuples -> flat list)
    flat_normals = []
    ...
    scene = {
        'asset': {'version': '2.0', 'generator': 'gen_local_glb.py'},
        'accessors': accessors,
...
def cube(cx, cy, cz, sx, sy, sz, color):
    ...
    fn = [(0,0,1)*4, (0,0,-1)*4, (0,1,0)*4, (0,-1,0)*4, (1,0,0)*4, (-1,0,0)*4]
    normals = [n for face in fn for n in face]
    indices = []
...
def generate_it_shop():
    ...
    parts.append(cube(-0.5, 1.5, 0.55, 0.4, 0.5, 0.05, 0x3a6b4f))
    parts.append(cube(0.5, 1.5, 0.55, 0.4, 0.5, 0.05, 0x3a6b4f))
...
SHOPS = {
    'fr': generate_fr_shop, 'jp': generate_jp_shop, 'mx': generate_mx_shop,
    'cn': generate_cn_shop, 'it': generate_it_shop, 'gb': generate_gb_shop,
    'us': generate_us_shop, 'nl': generate_nl_shop, 'gr': generate_gr_shop,
    'in': generate_in_shop, 'ma': generate_ma_shop, 'th': generate_th_shop,
}

_NAMES = {k: v.__doc__.split(':')[0].replace('generate_','').replace('_shop','').title() for k,v in SHOPS.items()}
_NAMES.update({
...
opts = '\n'.join(f'  {{src:"{g}", name:"{g.replace("_shop.glb","")}"}},' for g in glbs)
html = f'''<!doctype html><html lang="zh"><head><meta charset="utf-8"><title>3D Shop Models</title>
